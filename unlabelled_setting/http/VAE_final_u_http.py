...
def net(TRAIN_VAE = True, load_model = True, real_data=True, bayesian_opt = True, 
    ...
    epochs  = 51;              batch_size = 1024;       early_stopping = 30
    learning_rate = 3e-4;       l2_reg = 3e-2;          drop_probs = 0.25
    valid_every = 1;            anneal_rate = 5e-4;     n_test_steps = 100
    ...
    print("X-dim:",X_dim)
    # other
    num_batches = train_size//batch_size;       epoch = 0;      save_epochs = 0;    b_t_ratio = 5#train_size//batch_size
...
if __name__ == "__main__":
    ...
    precision, recall, f1, f01, tn, fn, tp, fp = ph.select_elbo_region(valid_elbo, test_elbo, y_test, 40, 60, fn_ratio=10)
    cnf_matrix = np.array([[int(tp), int(fn)],
                           [int(fp), int(tn)]])
    np.set_printoptions(precision=2)
    class_names = np.array(['Anomaly', 'Normal'], dtype='<U10')
