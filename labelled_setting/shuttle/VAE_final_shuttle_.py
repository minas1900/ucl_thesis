...
def net(TRAIN_VAE = True, load_model = True, real_data=True, bayesian_opt = True,
    ...
    ###############################################################################
    mode = "supervised"
    run_comment = mode+"_linVAElin_test"
    dataset = "Shuttle"
    ...
    epochs  = 251;              batch_size = 100;       early_stopping = 2
    learning_rate = 1e-4;       l2_reg = 1e-1;          drop_probs = 0.1
    val_epochs = 1;             anneal_rate = 5e-5;     n_test_steps = 10
    # sampling parameters & classification criterion
    num_z_samples = 10;          output_samples = 1;        kl_a = 1
    # load data directory
    #directory1 = 'Data/'
    ...
    # other
    num_batches = train_size//batch_size;       epoch = 0;      save_epochs = 0;    b_t_ratio = 1#train_size//batch_size
...
if __name__ == "__main__":
    conf_matrix,bo_results,n_elbo_valid,a_elbo_valid,n_elbo_test,a_elbo_test,res_np_file  =\
        net(TRAIN_VAE =False, load_model = True, real_data= True, bayesian_opt=True, anomaly_threshold=17,
            fp_ratio=1,fn_ratio=100,verbose=True, plots=True,batch_norm=False, dropout=True)
