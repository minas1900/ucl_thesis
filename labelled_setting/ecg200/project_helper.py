...
def standardize(x):
    ...
    """
    out = np.float32(np.zeros_like(x))
    x = np.float32(x)
    for i in range(x.shape[0]):
        temp = x[i] - np.min(x[i])
        out[i,:] = ((temp / np.max(temp)))
    return out
...
def saveres2txt(save_print_dir, file_name, dataset,anomaly_threshold, tp,fp,tn,fn,f1, auc,f01,precision,recall,
    ...
    dt = datetime.datetime.now().strftime("%y-%m-%d-%H-%M")
    mf.write("\n-------------------------------  Results ---------------------------------\n")
    mf.write("DataSet:\t{}\tAnomaly threshold:\t{}\tlearning rate:\t{}\n".format(dataset, anomaly_threshold,learning_rate))
    mf.write("tp:\t{}\tfp:\t{}\tfn:\t{}\ttn:\t{}\n".format(tp,fp,fn,tn))
    mf.write("AUC:\t{}\tf1-score:\t{}\tf0.1-score:\t{}\n".format(auc,f1,f01))
    mf.write("Precision:\t{}\tRecall:\t{}\n".format(precision,recall))
    mf.write("Validation set:\n")
    mf.write("Normal mean:\t{}\tNormal std:\t{}\n".format(ved['n_mean'],ved['n_std']))
    mf.write("Anomaly mean:\t{}\tAnomaly std:\t{}\n".format(ved['a_mean'],ved['a_std']))
    mf.write("Test set:\n")
    mf.write("Normal mean:\t{}\tNormal std:\t{}\n".format(ted['n_mean'],ted['n_std']))
    mf.write("Anomaly mean:\t{}\tAnomaly std:\t{}\n".format(ted['a_mean'],ted['a_std']))
    mf.close()
    return
...
def batch_normalizer(tensor, TrainTestFlag, exp_ma_rate=0.995, epsilon=1e-5):
    ''' Batch normalization with shift and bias
        During training it calculates the batch statistics, learns the gamma
        and beta parameters and normalizes using the TensorFlow operator.
        At the same time it performs an exponential moving average over the
        batch mean and variance which be used during prediction time for making.
        (with minor adaptions from:
         http://r2rt.com/implementing-batch-normalization-in-tensorflow.html)
    '''
    gamma = tf.Variable(tf.ones([tensor.get_shape()[-1]]))
    beta = tf.Variable(tf.zeros([tensor.get_shape()[-1]]))
    population_mean = tf.Variable(tf.zeros([tensor.get_shape()[-1]]), trainable=False)
    population_var = tf.Variable(tf.ones([tensor.get_shape()[-1]]), trainable=False)

    def f1():
        batch_mean, batch_var = tf.nn.moments(tensor, [0])
        print(batch_mean.get_shape())
        train_mean = tf.assign(population_mean,
                               population_mean * exp_ma_rate +
                               batch_mean * (1 - exp_ma_rate))
        train_var = tf.assign(population_var,
                              population_var * exp_ma_rate +
                              batch_var * (1 - exp_ma_rate))

        with tf.control_dependencies([train_mean, train_var]):
            return tf.nn.batch_normalization(tensor, batch_mean,
                                             batch_var, beta, gamma, epsilon) ,
    def f2():
        return tf.nn.batch_normalization(tensor,
                                         population_mean, population_var, beta, gamma, epsilon)

    ret = tf.cond(TrainTestFlag, f1, f2)
    return ret
...
def select_elbo_region(v_elbo, t_elbo, y_test,e_min,e_max,fn_ratio=10):
    ...
    length = t_elbo.shape[0]
    tn = sum([t_elbo[j] < e_min and y_test[j, 0] == 1 for j in range(length)])
    ...
    fn = sum([t_elbo[j] < e_min and y_test[j, 0] == 0 for j in range(length)])
    # calculate and report total stats
    # scores
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)
...
def score_stats(t_normal_elbo,t_anomaly_elbo,anomaly_threshold,fn_ratio=10):
    test_size = t_normal_elbo.shape[0]
    anom_test_size = t_anomaly_elbo.shape[0]
    ...
    f1 = 2 * precision * recall / (precision + recall)
    f01 = (1 + (1 / fn_ratio) ** 2) * precision * recall / ((1 / fn_ratio ** 2) * precision + recall)
    auc, _ = auc_plot(t_elbo, y_true, anomaly_threshold, f01)
    cnf_matrix = np.array([[int(tp), int(fn)],
                           [int(fp), int(tn)]])

    return tp,fp,tn,fn,precision,recall,f1,f01,auc, y_pred,y_true,cnf_matrix